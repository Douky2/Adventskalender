<script>
  ...
  import SideDecorations from '$lib/components/SideDecorations.svelte';
...
{#if data.musicFiles && data.musicFiles.length > 0}
  <MusicPlayer files={data.musicFiles} autoplay={data.musicAutoplay} />
{/if}
...
<style>
  :global(body) {
    ...
    /* Darker, cozier fireplace background */
    background-color: #1a0b05; 
    background-image: 
      radial-gradient(circle at 50% 40%, rgba(100, 40, 20, 0.5) 0%, rgba(26, 11, 5, 0.95) 80%),
